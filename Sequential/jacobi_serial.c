...
//        printf("Iteration %i", iterationCount);
            ...
            for (y = 1; y < (maxYCount-1); y++)
            {
                ...
                {
                    fX = xLeft + (x-1)*deltaX;
                    updateVal = ((u_old[(y)*maxXCount+(x-1)] + u_old[(y)*maxXCount+(x+1)])*cx + (u_old[(y-1)*maxXCount+(x)] + u_old[(y+1)*maxXCount+(x)])*cy + u_old[(y)*maxXCount+(x)]*cc - (-alpha*(1.0-fX*fX)*(1.0-fY*fY) - 2.0*(1.0-fX*fX) - 2.0*(1.0-fY*fY)))/cc;
                    // printf("updateval: %f\n",updateVal);
                    cur_error += updateVal*updateVal;
                    u[(y)*maxXCount+(x)] = u_old[(y)*maxXCount+(x)] - relax*updateVal;
                }
            }
            error = sqrt(cur_error)/((maxXCount-2)*(maxYCount-2));
        // }
//        printf("\tError %g\n", error);
        iterationCount++;
        // Swap the buffers
